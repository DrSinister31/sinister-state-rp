	...
	QBCore = {
		Functions = {
			GetPlayerData = function()
				local ply = exports['qbx_core']:GetPlayer(GetPlayerServerId(PlayerId()))
				return ply and ply.PlayerData or {}
			end,
			HasItem = function(item)
				return exports.ox_inventory and exports.ox_inventory:GetItemCount(item) > 0
			end,
			Notify = function(msg, type, duration)
				lib.notify({ title = msg, type = type or 'inform', duration = duration or 5000 })
			end,
		}
	}
else

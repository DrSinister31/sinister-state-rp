	...
	if Config.bgmusic and not pcall(xsound, res or false) then xSound = nil end
     while true do
          ...
          if NetworkIsSessionActive() or NetworkIsPlayerActive(PlayerId()) then
			...
			local ok = pcall(CharacterSelect)
			if not ok then
				DoScreenFadeIn(1000)
				print('^1[renzu_multicharacter] CharacterSelect failed, fade recovered')
			end
			break
          end
     end
...
RegisterNUICallback('nuicb', function(data, cb)
	local ok = pcall(function()
		...
		elseif data.msg == 'create' then
			...
			SetSkin(PlayerPedId(),skin)
			if (Config.framework == 'QBCORE' or Config.framework == 'QBX' or Config.framework == 'qbx') and GetResourceState('qb-spawn') ~= 'started' then
				SpawnSelect(vec4(defaultspawn.x,defaultspawn.y+10,defaultspawn.z,0.0))
				SkinMenu()
			end
		elseif data.msg == 'deletechar' then
			if Config.framework == 'QBCORE' or Config.framework == 'QBX' or Config.framework == 'qbx' then data.slot = characters[chosenslot].citizenid end
			callback('renzu_multicharacter:deletecharacter', data.slot)
			characters[chosenslot] = nil
			CharacterSelect()
			pedshots[chosenslot] = nil
		elseif data.msg == 'sex' then
